...
sub HomebridgeMapping
{
  ...
  $mapping .= "\nAirQuality=voc" if (ID($name,undef,'voc'));
  $mapping .= "\nCurrentAmbientLightLevel=luminance" if (ID($name,undef,'luminance'));
  $mapping .= "\nC53F35CE-C615-4AA4-9112-EBF679C5EB14=rainLevel,name=Regenmenge,minValue=0,maxValue=1000,minStep=0.001,format=FLOAT,unit=mm" if (ID($name,undef,'rainLevel'));
  $mapping .= "\n49C8AE5A-A3A5-41AB-BF1F-12D5654F9F41=wind,name=Windgeschwindigkeit,minValue=0,maxValue=200,minStep=0.1,format=FLOAT,unit=km/h" if (ID($name,undef,'wind'));
  $mapping .= "\n6B8861E5-D6F3-425C-83B6-069945FFD1F1=windGust,name=Böengeschwindigkeit,minValue=0,maxValue=200,minStep=0.1,format=FLOAT,unit=km/h" if (ID($name,undef,'wind'));
  $mapping .= "\n46F1284C-1912-421B-82F5-EB75008B167E=windDirection,name=Windrichtung,format=STRING" if (ID($name,undef,'wind'));
